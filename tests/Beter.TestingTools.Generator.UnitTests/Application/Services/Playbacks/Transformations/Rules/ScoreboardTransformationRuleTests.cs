...
{
    public class ScoreboardTransformationRuleTests
    {
        private static readonly Fixture _fixture = new();
        private readonly ScoreboardTransformationRule _rule = new();

        public ScoreboardTransformationRuleTests()
        {
            _fixture.Customizations.Add(new JsonNodeBuilder());
        }

        [Fact]
        public void IsApplicable_Returns_True_For_Scoreboard_MessageType()
        {
            // Arrange
        ...
        [Fact]
        public void IsApplicable_Returns_False_For_Non_Scoreboard_MessageType()
        {
            // Arrange
            var message = _fixture.Build<TestScenarioMessage>()
        ...
        [Fact]
        public void Transform_Calls_UpdateModel_Method_For_Each_ScoreboardModel()
        {
            // Arrange
        ...
        private static (ScoreBoardModel model, MessagesTransformationContext.MatchIdProfile profile) SetupModelAndProfile(string originalMatchId, DateTime originalDate, string newMatchId, DateTime oldFirstMessageTimestamp)
        {
        ...
        private static ScoreBoardModel SetupModel(string matchId, DateTime date)
        {
            return _fixture.Build<ScoreBoardModel>()
              .With(x => x.Id, matchId)
              .With(x => x.Timestamp, date.ToUnixTimeMilliseconds())
              .With(x => x.Timer, _fixture.Build<TimerModel>()
                .With(x => x.TimeStamp, date.ToUnixTimeMilliseconds())
                .Create())
              .Create();
        }
        ...
        private static MessagesTransformationContext.MatchIdProfile SetupProfile(string oldMatchId, string newMatchId, DateTime oldFirstMessageTimestamp)
        {
            var profile = _fixture.Build<MessagesTransformationContext.MatchIdProfile>()
                ...
                {
                    { MessageTypes.Scoreboard, oldFirstMessageTimestamp },
